...
<div style='display: flex; justify-content: center; gap: 50px; text-align: center;'>
    <div style='background-color: #e0f7fa; padding: 15px 0; border-radius: 15px; width: 180px; display: flex; flex-direction: column; justify-content: center; align-items: center; box-shadow: 2px 2px 10px rgba(0,0,0,0.1);'>
        <h3 style='margin: 0; font-size: 18px;'>🌞 Day Shift</h3>
        <h2 style='margin: 0; font-size: 32px;'>{len(day_df)}</h2>
    </div>
    <div style='background-color: #ffe0b2; padding: 15px 0; border-radius: 15px; width: 180px; display: flex; flex-direction: column; justify-content: center; align-items: center; box-shadow: 2px 2px 10px rgba(0,0,0,0.1);'>
        <h3 style='margin: 0; font-size: 18px;'>🌙 Night Shift</h3>
        <h2 style='margin: 0; font-size: 32px;'>{len(night_df)}</h2>
    </div>
    <div style='background-color: #d1c4e9; padding: 15px 0; border-radius: 15px; width: 180px; display: flex; flex-direction: column; justify-content: center; align-items: center; box-shadow: 2px 2px 10px rgba(0,0,0,0.1);'>
...
with tabs[0]:
    ...
    st.markdown(
        "<div style='background-color: #e0f7fa; padding: 15px; border-radius: 10px; width: 95%; box-shadow: 2px 2px 10px rgba(0,0,0,0.1);'>",
...
with tabs[1]:
    ...
    st.markdown("<div style='display: flex; justify-content: center; margin-top: 10px;'>", unsafe_allow_html=True)
    st.markdown(
        "<div style='background-color: #fff3e0; padding: 15px; border-radius: 10px; width: 95%; box-shadow: 2px 2px 10px rgba(0,0,0,0.1);'>",
        unsafe_allow_html=True
    )
...
with tabs[2]:
    ...
    # Day Shift
    col_day.markdown("<div style='background-color:#e0f7fa; padding:10px; border-radius:10px;'>### 🌞 Day Shift</div>", unsafe_allow_html=True)
    for i, row in day_df.iterrows():
        name = row["Name"]
        attendance[name] = col_day.checkbox(name, key=f"day_emergency_{i}")

    # Night Shift
    col_night.markdown("<div style='background-color:#fff3e0; padding:10px; border-radius:10px;'>### 🌙 Night Shift</div>", unsafe_allow_html=True)
